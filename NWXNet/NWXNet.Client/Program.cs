    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                var response = NWX.Request.For(TAF.ForICAO("CYYC")).Send();
                //var response = NWX.Request.For(Available.GeoMagModels).Send();//Chart.For("46,-150", "63,-54", 700, epoch, 1280, 1024).WithId("chart1").WithFeatures(ChartFeatures.SurfaceTemperature())).Send();
               
                if (!response.HasWarnings)
                {
                    Console.WriteLine(response.WithId<TAFResponse>("CYYC"));
